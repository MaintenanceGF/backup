...
function create() {
    ...
    const data = `INSERT INTO requisiciones (requisition, po, status, line, description, supplier, requester, buyer, arrived_date, tracking, progress, delivery, note) VALUES (${requisition}, 0, " ", "${line}", "${description}", "${supplier}", "${requester}", "${buyer}", " ", "Sin PO", 0, "En tiempo", "${note}")`
